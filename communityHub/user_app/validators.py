...
def increment_fail_count(cache_key, timeout=settings.LOCK_TIME):
    """ 增加失败计数 """

    current_count = get_fail_count(cache_key)
...
def check_ip_lock(request):
    ...
    ip_fail_count = get_fail_count(ip_fail_key)
...
def check_account_lock(account):
    ...
    account_fail_count = get_fail_count(account_fail_key)
...
def record_login_failure(request, account):
    ...
    increment_fail_count(ip_fail_key)
    increment_fail_count(account_fail_key)
